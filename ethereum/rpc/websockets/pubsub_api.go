...
func NewAPI(clientCtx client.Context, log log.Logger) *PubSubAPI {
	return &PubSubAPI{
		clientCtx: clientCtx,
...
func (api *PubSubAPI) subscribeSyncing(conn *websocket.Conn) (rpc.ID, error) {
	...
	status, err := api.clientCtx.Client.Status(context.Background())
	if err != nil {
	...
	go func(headersCh <-chan coretypes.ResultEvent, errCh <-chan error) {
		for {
			...
			case <-headersCh:
				...
				newStatus, err := api.clientCtx.Client.Status(context.Background())
				if err != nil {
