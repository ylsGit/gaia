...
func newCommitStateDB(
	ctx sdk.Context, storeKey sdk.StoreKey, paramSpace paramtypes.Subspace, ak AccountKeeper, bk BankKeeper,
) *CommitStateDB {
...
func (csdb *CommitStateDB) updateStateObject(so *stateObject) error {
	...
	newBalance := sdk.Coin{Denom: evmDenom, Amount: sdk.NewDecFromBigIntWithPrec(so.Balance(), sdk.Precision).RoundInt()} // int2dec
	if !newBalance.IsValid() {
		return fmt.Errorf("invalid balance %s", newBalance)
	}

	//checking and reject tx if address in blacklist
	if csdb.bankKeeper.BlockedAddr(so.account.GetAddress()) {
		return fmt.Errorf("address <%s> in blacklist is not allowed", so.account.GetAddress().String())
	}

	if err := csdb.bankKeeper.SetBalance(csdb.ctx, so.account.GetAddress(), newBalance); err != nil {
		return err
	}
...
func (csdb *CommitStateDB) CreateAccount(addr ethcmn.Address) {
	...
	if prevobj != nil {
		newobj.setBalance(sdk.NewDecFromBigIntWithPrec(prevobj.Balance(), sdk.Precision)) // int2dec
	}
}
