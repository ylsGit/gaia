...
func (st StateTransition) TransitionDb(ctx sdk.Context, config ChainConfig) (exeRes *ExecutionResult, resData *MsgEthereumTxResponse, err error) {
	defer func() {
		if e := recover(); e != nil {
			...
			if blockedContractAddr, ok := e.(common.Address); ok {
				err = sdkerrors.Wrapf(ErrCallBlockedContract, "the contract %s is not allowed to invoke", blockedContractAddr.Hex())
			} else {
	...
	case true:
		...
		if params.EnableContractDeploymentWhitelist && !csdb.IsDeployerInWhitelist(senderAccAddr) {
			return exeRes, resData, sdkerrors.Wrapf(ErrUnauthorizedAccount, "the account %s is not allowed to deploy a contract", common.BytesToAddress(senderAccAddr))
		}
	...
	resData = &MsgEthereumTxResponse{
		Bloom:  bloomFilter.Bytes(),
		Logs:   NewLogsFromEth(logs),
		Ret:    ret,
		TxHash: st.TxHash.String(),
	}

	if contractCreation {
		resData.ContractAddress = contractAddress.String()
	}

	resBz, err := EncodeTxResponse(resData)
	if err != nil {
		return
	}
